...
const fs = require("fs");
const config = require(path.join(global.cwd, "app.json"));
...
const compile = (type, object) => {
    ...
    const componentDir = path.join(global.cwd, 'src', type.toLowerCase() + 's', object + '.yaspa');
    const parentDir = path.join(global.cwd, 'src', type.toLowerCase() + 's');
